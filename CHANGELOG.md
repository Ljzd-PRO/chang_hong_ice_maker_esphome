...
本版本将 Home Assistant 的电源开、关请求改为有界 FIFO 队列。快速连续切换 `Power` 时，固件不再因为前一个动作正在脉冲、冷却或等待面板状态确认而直接丢弃后续电源目标，而是按用户点击顺序逐项执行。

Home Assistant 的 `Power` 开关会显示队列中最后一个目标，也就是用户最终要求的状态。每个目标仍需等待真实面板状态确认后才会派发下一项，因此队列不会在短时间内盲目连续发送开关键脉冲。

### 新增

- 新增最多 4 项的电源目标 FIFO 队列。
- 新增 `queued_power_*`、`skipped_power_*_already_satisfied`、`refused_power_queue_full` 等动作诊断日志。
- 新增独立 C++ 队列单元测试，并接入 GitHub Actions 测试工作流。

### 行为说明

- 连续重复的同一电源目标会合并，不产生重复按键。
- 队列中的目标如果已被当前真实状态满足，会被安全跳过。
- 电源队列优先于尚未开始的 UV 队列；正在执行的脉冲不会被中断。
- 电源队列执行期间，大小冰切换仍会被拒绝，避免控制动作交叉。
- 队列满后拒绝新增电源请求，并通过 `Action Result` 和日志报告原因。
...
这是一次数据库减负版本。固件主控制逻辑、GPIO 接线、状态识别算法和 Home Assistant 主实体不变；更新重点是降低高频诊断实体对 Home Assistant recorder 历史数据库的长期写入压力。

升级后，新添加设备时，高频诊断实体默认不会自动启用。需要排查接线或分类问题时，可在 Home Assistant 设备页面手动启用对应实体；排查结束后建议再次禁用。

已在旧版本中启用过的诊断实体，Home Assistant 可能会保留用户既有启用状态。若升级后仍看到这些实体持续写入历史，需要在 HA 中手动禁用，或通过 `recorder` 配置排除。

### 变更

- 主固件中以下高频诊断实体默认禁用：
  - 内部分类状态和短窗口候选状态。
  - ADC 签名、ratio、confidence、blink score。
  - 小冰/待机特征得分、`P2 StdDev`、支路 delta。
  - `P1 Raw` 到 `P5 Raw`。
- 调试固件版本同步为 `1.0.1-debug`，但调试实体保持默认启用，便于短时数据采集。
- README 和英文 README 增加说明：高频诊断实体默认禁用，以及已有 HA 安装可能需要手动禁用或 recorder 排除。

### 不变

- `Power`、`Large Ice`、`UV Toggle`、`State` 主实体保持默认启用。
- Release 固件的固定 `api_encryption_key`、Fallback AP 密码和 OTA 密码保持不变。
- 当前仍只声明适配并验证长虹 `CH-Z6Y3`。
