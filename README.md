...
连续切换 `Power` 时，固件会按点击顺序排队执行开、关目标，队列最多保留 4 项；连续重复的同一目标会自动合并。Home Assistant 中的开关显示队列最后一个目标，因此代表用户最终要求的状态。每一项都会等上一项由面板状态确认后再执行，请勿把该功能用于持续快速启停制冰机。
...
- UV 没有状态反馈，只能提供翻转按钮。
- 电源开关请求按顺序排队，最多 4 项；队列满后会拒绝新增请求。大小冰请求在电源队列执行期间仍会被拒绝。
